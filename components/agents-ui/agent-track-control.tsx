...
type TrackDeviceSelectProps = React.ComponentProps<'select'> &
...
function TrackDeviceSelect({
  ...
  const formatDeviceLabel = (label: string) => {
    // Remove long technical descriptions in parentheses
    let formatted = label.replace(/\(.*?\)/g, '').trim();
    // If still very long, truncate
    if (formatted.length > 50) {
      formatted = formatted.substring(0, 50) + '...';
    }
    // Default to kind if empty
    return formatted || (kind === 'audioinput' ? 'Microphone' : 'Camera');
  };
  ...
  return (
    <select
      ...
      className={cn(selectVariants({ size, variant }), className, 'rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground')}
      {...props}
    >
      {size !== 'sm' && (
